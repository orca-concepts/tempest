...
**Current Status:** Phase 69 complete. **Concept-card & links-panel enhancements** (4 surfacing features, all display of data that already existed): (1) a cross-sibling-descendant flag — a `↘` arrow on a child card when that child also lives deeper in the same parent's subtree (under a sibling), clicking to the nested location or a picker for >1; (2) a per-card link count next to the children count (on both the children grid and the root grid); (3) alt-parent / tunnel-link counts appended to the Flip / Tunnel buttons when > 0; (4) vote-set color dots on links — a link gets a set's dot when ≥1 member of that set upvoted it (mirrors the child-card dots, gated on the same 10-user vote-set threshold). Phase 68 (ORCID badge validation + `chaos-seed-data` rename), Phase 67 (situations-as-tabs + votes), and Phases 66, 64a, 65a, 65b-1/2, 62c, 63a remain live.
