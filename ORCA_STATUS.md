...
- **58c-1** — Frontend pure deletion. `git rm` 19 retired components: `CorpusTabContent.jsx`, `CorpusListView.jsx`, `CorpusDetailView.jsx`, `CorpusDocumentList.jsx`, `CorpusUploadForm.jsx`, `CorpusMembersPanel.jsx`, `DocumentView.jsx`, `AnnotationPanel.jsx`, `AnnotateFromGraphPicker.jsx`, `AnnotationWarningModal.jsx`, `AnnotationVotesOverlay.jsx`, `CitationRedirect.jsx`, `MessagesPage.jsx`, `MessageThread.jsx`, `AcceptInvite.jsx`, `DocInviteAccept.jsx`, `OrphanRescueModal.jsx`, `InfringementNoticePage.jsx`, `CounterNoticePage.jsx`. Remove all imports of these components from any file that imports them, even if other usages of those files (like JSX rendering) get fixed in 58c-2. Frontend will be temporarily broken (build errors / missing components) — that's expected and gets fixed in 58c-2.
- **58c-2** — Frontend surgical edits. Edits to `AppShell.jsx` (heavy: corpus state, message state, annotation state, pending document/annotation logic, sidebar buttons, overlay rendering, LEGAL_SLUGS), `Concept.jsx` ("Add as Annotation" button + AnnotateFromGraphPicker rendering), `ConceptAnnotationPanel.jsx` (strip Annotations tab, keep Web Links + Superconcepts tabs; rename and tab restructure deferred to 58d), `ComboTabContent.jsx` (strip annotation list/voting/sort options, keep combo CRUD/subscriptions/edges; aggregated links view added in 58d), `App.jsx` (remove `/invite/:token`, `/doc-invite/:token`, `/cite/a/:annotationId` routes), `LegalPage.jsx` (remove copyright reporting form section), `SearchField.jsx` (remove corpus annotation badges), `SavedPageOverlay.jsx` (remove corpus badges), `DeleteAccountFlow.jsx` (remove corpus transfer pre-check, keep combo), `AdminLegalRemovalsPanel.jsx` (remove infringement/counter-notice sections, document/annotation target types). After 58c-2 the frontend builds cleanly via `npm run build` and runs locally.
