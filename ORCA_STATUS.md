...
*Separate workstream from the app phases above. Chaos is the graph-seeding system that authors the initial Orca `value` taxonomy and grounds it in real research. The design spec lives in `chaos.md` (currently **v2.0**, adjective-constrained); this section tracks build/operational state only. Nothing below has been written to the Orca dev-graph DB yet — Chaos remains **pre-cutover**.*

**Model (v2.0, adjective-constrained).** Nodes are bare quality-adjectives describing good research ("Honest", "Transparent", "Reproducible") under an implicit "Good" predicate. Meaning is compositional (read along the path) and carried by multi-parent placement — the same adjective under different parents is a distinct contextual entity. The v1.0 "conduct"/lived-rendering face was removed: there is no per-node prose and no `description`/`conduct` column on `concepts`. Comprehension of abstract nodes comes from aggregated instances (see upward aggregation, in Next). Rationale: `chaos.md §0`.

**Three engines** (named for the order that emerges out of Chaos):

- **Ouranos — the Categorizer** (`chaos/categorizer.js`). **BUILT** (genesis mode). Loads `chaos.md` as its system prompt; emits `chaos/genesis/proposal.json` + `proposal.md`. The v2.0 genesis run produced a **61-node, 10-root adjective taxonomy** — roots: Honest, Rigorous, Careful, Transparent, Open, Reliable, Fair, Humble, Significant, Clear — adjective-form throughout, ~30% of nodes multi-parented. Primary admission discipline is the **contrastive-separability gate** (admit a child only if research can be {parent}-but-not-{child} AND {child}-but-not-{sibling}); this guards against thesaurus/synonym piles. Structural checks: `chaos/genesis/validate.js`. Genesis is currently one-shot (no frontier-driven run mode yet — see Next).
- **Gaia — the Instantiator** (`chaos/gaia.js`). **BUILT + RUN, two passes:**
  - **Primary grounding** (`--all`, `--node`, `--limit`). For each LEAF adjective (once per path), retrieves open-access papers from OpenAlex, judges **"exhibits not discusses"** (the paper's own research must *enact* the quality, not *study it as a topic*), and emits links whose comments name the concrete exhibiting feature in **plain, cross-disciplinary, jargon-free** language. **Result: 50 / 54 leaf targets grounded, 141 instance links.** Output: `chaos/genesis/gaia-proposal.json` + `.md`.
  - **Multi-attach** (`--multiattach`). One paper → many concepts. Re-reads each *accepted* paper (full text already on disk) against the whole leaf menu to catch qualities it exhibits beyond the one it was retrieved for — the proper grounding mechanism for buried-practice qualities that topic search can't surface (consent, blinding, power, etc. live one sentence deep in methods). Two stages: **Stage A (propose)** — one cheap call per paper proposes candidate qualities, each requiring a concrete locatable feature (the *feature-or-abstain* gate; abstains on interpretive/tonal-only reads); **Stage B (confirm)** — the **same strict `judgePaper`** confirms each candidate (no new lax judgment path; its per-(target,paper) cache also gives free dedupe against the primary pass). **Result: 97 full-text accepted papers scanned (34 abstract-only skipped) → 500 new instance links across 47 concepts.** Output: `chaos/genesis/gaia-multiattach-proposal.json` + `.md` (separate + reviewable; merges with the primary `links[]` at cutover).
- **Krius — the Scout** (`chaos/scout.js`). **NOT BUILT.** Bottom-up channel to propose new frontiers / adjectives / multi-parent placements within the existing spine.

**Gaia operational details (hard-won — do not rediscover):**
- **OpenAlex now requires an API key** (policy changed Feb 2026; the old polite-pool `mailto` parameter is ignored). Keyless tier ≈ **$0.10/day ≈ ~100 searches**; a **free API key gives ~$1/day ≈ ~1,000 searches** — enough for a full run (~160 searches) several times over. Get a free key at openalex.org → settings/api. A full primary run *without* a key WILL exhaust the budget mid-run. (Multi-attach uses NO OpenAlex — Anthropic calls only — so it has no rate-limit exposure.)
- **A 429 means one of two different things.** *Too-fast* (temporary) vs. *daily credits exhausted* (permanent until **midnight UTC** = 8 PM Dayton time). Gaia distinguishes them: an ordinary 429 → capped exponential backoff (2/5/15/30s, **hard-capped at 60s** so a large `Retry-After` can't freeze the run for hours); a credit-exhaustion 429 → **halts the run** with a clear "resets at midnight UTC" message rather than retry-spamming every remaining leaf.
- **Leaf-level cache** (`chaos/gaia_cache/`, gitignored). A fully-grounded leaf is cached and **skipped entirely on re-run — including its OpenAlex search.** A leaf whose searches 429'd is **not** cached, so it retries next run. Per-paper judgments, per-node query plans, and (multi-attach) per-paper propose results are all cached too; everything is resumable on Ctrl+C. The cache auto-invalidates when `chaos.md` (rubricHash) or `GAIA_PROMPT_VERSION` changes. **Multi-attach uses a SEPARATE `GAIA_MULTIATTACH_VERSION` ('ma1')** so its prompt can change without invalidating the primary grounding caches.
- **Multi-attach distribution is healthy, not runaway:** mean ~5.3 attachments/paper, median 5, **max capped at 8** (the Stage-A candidate cap). The Open/Transparent family stays *discriminated* rather than rubber-stamped (Shared 23 / Accessible 22 / Reusable 11 — the spread shows the gate distinguishing findable-vs-downloadable-vs-licensed-for-reuse, not stamping the whole cluster). Caveated is the top concept (55) because a limitations section is near-universal and concretely pointable — expected, not inflated.
- **Env vars:** `OPENALEX_API_KEY` (**never hardcode — the repo is public**; pass via the terminal), `CHAOS_OPENALEX_DELAY_MS` (inter-search pacing, default ~1000ms; raise to slow down further), `CHAOS_MAILTO`. Fetched papers cache to `chaos/papers/<id>.json` (gitignored).
- **Run:** primary — `set OPENALEX_API_KEY=… && node chaos/gaia.js --all`; multi-attach — `set ANTHROPIC_API_KEY=… && node chaos/gaia.js --multiattach`; `--plan` for an offline target list.

**Zero-accept paths — mostly resolved by multi-attach.** The primary pass left 4 leaf-paths with no direct instances; multi-attach (grounding them off papers retrieved for *other* qualities) filled 3:
- `Consenting` under Fair › Respectful: **0 → 28.** The gate was confirmed *correct, not over-strict* — all ~40 primary rejections were papers *about* consent; topic search structurally couldn't surface ordinary studies that simply obtained consent in their methods. Multi-attach catches them off real studies. The purest demonstration of why multi-attach exists.
- `Consistent` under Reliable: **0 → 14.** `Precise` under Reliable: **0 → 3.** Both grounded under their *other* parent in the primary pass; the abstract-sense path filled via cross-attachment.
- `Precise` under Clear: **still 0** — the "unambiguous language" sense has no single pinpointable feature a paper exhibits. Expected to fill via **upward aggregation** (app-side), not direct or cross grounding.

**Built Feature affecting Chaos** Upward link aggregation (built, app-side). getWebLinks (votesController.js) now shows a concept's directly-placed links plus all links on descendant edges along the same graph_path lineage — path-scoped, not concept-id-scoped, so a link at A›B›C surfaces at B and A but never leaks to a different placement of B or C. Same prefix-scan in the flip view and the per-child/per-root count badges. No edges.graph_path index yet (fine at seed scale; revisit before large growth). Agrees with apply.js's path→edge resolution, so seed links will aggregate correctly at cutover.

**Next (Chaos):**
1. **Cutover wiring** *(top remaining substantive step)* — fold BOTH proposals' `links[]` (primary + multi-attach) and their fetched `papers[]` into the combined genesis write via `chaos/genesis/write.js` → `apply.js`. The link write + path→edge resolution already exist in `apply.js`, and both proposals' link shape already matches `{ attribute, concept_name, parent_path, url, claim, paper_id }`. Then clear the dev graph and write concepts + edges + links together. Pre-cutover housekeeping: trim `apply.js`'s v0.x payload (tunnels / papers / predictions / precision ledger) per the pure-hierarchy design, and retire `chaos/reason.js` + `chaos/source.js`.
2. **Broaden full-text coverage** (future grounding-quality pass). Corpus full-text coverage is ~46% — `fullTextPlan` only fetches from Europe PMC / arXiv / bioRxiv / medRxiv / PMC / PLOS / eLife / Frontiers / MDPI; OA papers on other publishers fall back to abstract-only, which is weaker evidence for methods-buried qualities. Pair with re-fetching the ~34 abstract-only accepted papers and re-judging (and re-running multi-attach over them, since multi-attach skips no-full-text papers).
3. **Multi-attach yield leak (minor).** A handful of Stage-B confirm responses returned unparseable JSON and were dropped (`(warn) judgment unparseable … not cached`), losing a few legitimate attachments per run. Not corrupting; retried (not reused) on re-run. If it recurs at volume, add a one-shot reparse/retry on unparseable confirm responses.
4. **Krius (Scout)** — build the third engine.
5. **Frontier object + frontier-driven run mode** in Ouranos (genesis is currently one-shot).

**Key files:** `chaos.md` (spec, v2.0) · `chaos/categorizer.js` (Ouranos) · `chaos/gaia.js` (Gaia: primary + `--multiattach`) · `chaos/genesis/proposal.json` + `.md` (taxonomy) · `chaos/genesis/gaia-proposal.json` + `.md` (primary grounding, uncommitted) · `chaos/genesis/gaia-multiattach-proposal.json` + `.md` (multi-attach grounding, uncommitted) · `chaos/genesis/validate.js` · `chaos/apply.js` (writer, transitional — still carries v0.x payload) · seed account `chaos-seed-data` (resolved by username, id 25 locally).
