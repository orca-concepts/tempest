...
**Current Status:** Phase 65b-1 complete (with `65b-1-fix` follow-up). Mentions data layer is live: new `comment_mentions` join table indexes in-orca URLs in comments and addenda across four source surfaces; `parseMentions` is hooked into all four write paths and inserts within the same transaction as the source row; a new `GET /api/mentions/:targetType/:targetId` endpoint reads mentions filtered by visibility (hides mentions whose source's parent link is hidden / legal-held); an idempotent backfill script (`backend/scripts/backfill-mentions.js`) scanned existing comments locally and in production. The 65b-1-fix follow-up resolved a design hole: root concepts have empty `graph_path` and the share button was omitting `?path=` entirely, which the strict mention parser rejected as ambiguous; the share button now always emits `?path=` (with empty value for roots), and the parser distinguishes "path absent" (reject) from "path empty" (accept as root reference). Also resolved a Phase 65a hangover, `65a-fix-3`: opening a `/tunnel/:id` share URL no longer renders the destination concept in the wrong (default) view mode — AppShell now passes `viewMode: 'tunnel'` to `handleOpenConceptTab`, using the function's existing positional argument. **No Mentions UI ships in this phase** — Phase 65b-2 will add the Mentions panels on concepts, link cards, and tunnel cards, plus `mentionCount` in existing list endpoints. Phase 64a, 65a, 62c, and 63a remain live.
...
GET `/:targetType/:targetId?path=&limit=&offset=` — paginated mentions filtered by visibility. Guest-accessible. `targetType` must be one of `concept`, `superconcept`, `link`, `tunnel` (400 on invalid). `targetId` must be a positive integer (400 on invalid). For `concept` targets `?path=` is required (may be empty for root concepts — see Phase 65b-1-fix); ignored for link/tunnel/superconcept targets. `limit` defaults to 20, max 50; `offset` defaults to 0. Visibility filter is a LEFT JOIN to source/parent tables that hides mentions whose source's parent link/tunnel is `is_hidden = true` or `legal_hold = true`; un-hiding restores them automatically. Response shape: `{ mentions: [{ id, sourceType, sourceId, sourceParentId, sourceParentType, sourceText, sourceAuthor: { id, username, orcidId }, createdAt }], totalCount, hasMore }`. Sort order: newest first by mention `created_at`.
...
- Production: `^https://orcaconcepts\.org/(concept|superconcept|link|tunnel)/[0-9]+(\?path=[0-9,]*)?(#(link|tunnel)-[0-9]+)?$`
...
- Phase 65b-2: Mentions UI — Mentions panel on concepts (next to Links/Superconcepts tabs), Mentions expandable on regular link cards (alongside the existing "Other instances" buttons), Mentions expandable on tunnel cards. `mentionCount` added to existing list endpoints (`/web-links/:edgeId`, concept response, tunnel response) so badges display without an extra round trip. Pagination at 20 with Load More. Mentions show author + ORCID badge + clamped snippet + parent context; click opens the source's parent link in a new browser tab via the 65a share URL pattern. **Next phase to ship.**
...
## Recent Commits (Phase 59 through 65b-1)
