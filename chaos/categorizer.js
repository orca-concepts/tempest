 ...
 *   node chaos/categorizer.js                 # genesis pass at default effort
 *   node chaos/categorizer.js --effort=max
 *   node chaos/categorizer.js --force         # ignore the cache, re-call the model
 *
...
async function main() {
  ...
  console.log('Chaos categorizer — GENESIS MODE: authoring the initial research-values taxonomy from scholarship.');
...
if (require.main === module) {
  main()
    ...
    .catch(async (err) => {
      console.error('\ncategorizer.js failed:', err.message);
      await closeHttp();
      process.exitCode = 1;
    });
}
