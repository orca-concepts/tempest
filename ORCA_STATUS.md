...
- **Gaia — the Instantiator** (`chaos/gaia.js`). **BUILT + RUN.** Takes each leaf adjective (once per path), retrieves open-access papers from OpenAlex, judges **"exhibits not discusses"** (the paper's own research must *enact* the quality, not *study it as a topic*), and emits links whose comments name the concrete exhibiting feature in **plain, cross-disciplinary, jargon-free** language. Output: `chaos/genesis/gaia-proposal.json` + `.md` (reviewable; intentionally **not committed**). **Latest full run: 50 / 54 leaf targets grounded, 141 instance links.**
...
- **OpenAlex now requires an API key** (policy changed Feb 2026; the old polite-pool `mailto` parameter is ignored). Keyless tier ≈ **$0.10/day ≈ ~100 searches**; a **free API key gives ~$1/day ≈ ~1,000 searches** — enough for a full run (~160 searches) several times over. Get a free key at openalex.org → settings/api. A full `--all` run *without* a key WILL exhaust the budget mid-run.
...
- **Leaf-level cache** (`chaos/gaia_cache/`, gitignored). A fully-grounded leaf is cached and **skipped entirely on re-run — including its OpenAlex search.** (The search is otherwise unguarded and fires on every run; that re-searching is what repeatedly tripped the rate limit during development.) A leaf whose searches 429'd is **not** cached, so it retries next run. Per-paper judgments and per-node query plans are cached too. The cache auto-invalidates when `chaos.md` (rubricHash) or `GAIA_PROMPT_VERSION` changes.
...
- **Run:** `set OPENALEX_API_KEY=… && node chaos/gaia.js --all` · `--node "<Name>"` or `--limit N` to test small · `--plan` for an offline target list (no network/model).

**The 4 zero-accept paths** (searched + judged, but accepted nothing):
- `Precise` under Clear · `Precise` under Reliable · `Consistent` under Reliable — **correct path-level refusals.** Each of these adjectives grounded under its *other* parent (Precise under Rigorous › Calibrated; Consistent under Rigorous › Warranted), where the sense is concretely demonstrable. The empty paths are the abstract senses (unambiguous *language*, low *variance*, stable *findings*) that papers rarely enact in a single pinpointable feature. Expected to fill via **upward aggregation**, not direct retrieval — so no fix needed.
- `Consenting` under Fair › Respectful — **possible over-strictness.** Consent is concretely enacted in many studies, so an empty result here is mildly surprising; the gate may be treating routine informed-consent statements as too-boilerplate to count (the way it correctly rejects boilerplate conflict disclosures). Flagged for read-only review (see Next).

**Next (Chaos):**
1. 1. **`Consenting` resolved (gate is correct, not over-strict).** Reviewed all ~40 rejections: every one is a paper *about* consent (comprehension studies, template surveys, consent-tool development), correctly rejected. The gap is retrieval, not judgment — topic search returns the consent-*methodology* literature, while real exemplars (studies that simply obtained consent in their methods) never surface. This is the purest buried-practice case and the strongest argument for multi-attach (item 2): Consenting should ground for free off studies retrieved for other qualities (e.g. the vulnerable-participant studies already grounded under `Protective`).
2. **Multi-attach** — one paper → multiple concepts. The proper grounding mechanism for buried-practice leaves (catch a quality as a *secondary* feature of a paper retrieved for its primary quality, rather than topic-searching for it directly). Promoted from "later" to a real priority by the buried-leaf retrieval cost observed during the Gaia run.
3. **Upward link aggregation** (app-side) — a link added at path A › B › C also surfaces at edges B and A *along that path*. Makes abstract nodes rich-by-aggregation and fills the 3 correct-refusal empty paths above. Moderate effort: `getWebLinks` moves from `edge_id = $1` to `edge_id = ANY(<ancestor edge ids>)`; the `graph_path` prefix arithmetic already exists in `getAllVotes`.
4. **Cutover wiring** — fold Gaia's `links[]` (and fetched `papers[]`) into the combined genesis write via `chaos/genesis/write.js` → `apply.js`. The link write + path→edge resolution already exist in `apply.js`, and the proposal link shape already matches `{ attribute, concept_name, parent_path, url, claim, paper_id }`. Then clear the dev graph and write concepts + edges + links together. Pre-cutover housekeeping: trim `apply.js`'s v0.x payload (tunnels / papers / predictions / precision ledger) per the pure-hierarchy design, and retire `chaos/reason.js` + `chaos/source.js`.
5. **Krius (Scout)** — build the third engine.
6. **Frontier object + frontier-driven run mode** in Ouranos (genesis is currently one-shot).

**Key files:** `chaos.md` (spec, v2.0) · `chaos/categorizer.js` (Ouranos) · `chaos/gaia.js` (Gaia) · `chaos/genesis/proposal.json` + `.md` (taxonomy) · `chaos/genesis/gaia-proposal.json` + `.md` (grounding, uncommitted) · `chaos/genesis/validate.js` · `chaos/apply.js` (writer, transitional — still carries v0.x payload) · seed account `chaos-seed-data` (resolved by username, id 25 locally).
