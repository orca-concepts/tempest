...
**Current Status:** Phase 68 complete. **ORCID badge display validation** + seed-account rename. System accounts carry a non-ORCID sentinel `orcid_id` (to satisfy `NOT NULL`); the UI rendered it as a badge linking to `orcid.org/<sentinel>`, which 400s. A new shared format validator nulls invalid iDs at the badge-display emit points and renders no badge for non-ORCID values (display-only; the stored sentinel and the `NOT NULL` constraint are untouched). The Chaos seed account was renamed `chaos-seed` → `chaos-seed-data` (username + email), renamed in place by `migrate-chaos.js` preserving its id and all attribution. Phase 67 (situations-as-tabs + situation votes) and Phases 66, 64a, 65a, 65b-1/2, 62c, and 63a remain live.
