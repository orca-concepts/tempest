...
**Sub-phase plan:**
- **58.0** — New GitHub repo + deletion inventory. Old repo deleted; new `orca-concepts/orca` repo to be created. Push current code to it as starting point. Run a Claude Code prompt that greps the codebase and produces `PHASE_58_DELETION_INVENTORY.md` cataloging every file/route/component/table that touches retired features. Do NOT reconnect Railway yet.
- **58a** — Database migration. Drop tables: `documents`, `corpuses`, `corpus_documents`, `document_annotations`, `annotation_votes`, `annotation_color_set_votes`, `corpus_subscriptions`, `corpus_allowed_users`, `document_authors`, `document_tags`, `document_tag_links`, `document_favorites`, `document_citation_links`, `document_subscriptions`, `combo_annotation_votes`, `annotation_removal_log`, messages tables. Drop user column `hide_annotation_warning`. Trim `sidebar_items` rows for retired item types. Update `migrate.js`. Pre-drop SQL backup step included.
- **58b** — Backend API removal + new endpoints. Remove all controllers/routes for documents, corpuses, annotations, citations, messages, document tags, copyright report form. Add `GET /api/concept-links/by-url?url=<encoded>` for cross-concept search. Add `?sort=new|top` to `GET /web-links/:edgeId`. Add server-side OG title fetcher with SSRF protections. Trim data export endpoint. Update account-deletion pre-checks (no more "transfer corpuses first").
- **58c** — Frontend removal. Delete corpus/document/annotation/messages/citation UI components. Remove sidebar buttons (Browse Corpus, Messages). Remove pending document/annotation cross-component navigation logic from `AppShell.jsx`. Remove Copyright reporting form on legal page. Remove "Add as Annotation" button from `Concept.jsx`. Remove `MobileBlocker.jsx` (Phase 57 abandoned).
- **58d** — Frontend new links UI + cross-instance browser. Rename `ConceptAnnotationPanel.jsx` → `ConceptLinksPanel.jsx`. Tabs become Links | Superconcepts. Sort toggle Top | New. "Show other instances on this concept" + "Show instances across all concepts" expandable buttons with snippets. Update Graph Votes page → Link Votes browser with click-through-and-scroll.
- **58e** — Legal cleanup, ORCA_STATUS pruning, history migration. Remove Copyright reporting form references from legal page. Migrate completed phases through Phase 56 from `ORCA_STATUS.md` → `ORCA_HISTORY.md`. Rewrite `ORCA_STATUS.md` to reflect new simplified scope. Update `CLAUDE.md` if it references retired conventions.
