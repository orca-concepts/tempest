...
**Next (Chaos):**
1. **Review + selectively apply Krius placements.** Read `krius-proposal.md`; for the 36 writable `missing_placement` frontiers, apply the approved ones via the name-based `apply.js` multi-parent path (human-approved, one at a time). Give borderline placements an extra look (e.g. Truthful › Conservative). Adjectives, abstractions, and possible-merges are advisory — take up the strong ones (Forthcoming, Verifiable, Restrained) as Ouranos frontiers or hand placements.
