...
3. **Full-text coverage pass — DONE (June 27, 2026).** Added OA-html / Unpaywall / PDF fallback fetchers to Gaia's full-text subsystem + a read-only `--coverage-probe`; lifted coverage ~47% → ~56% on the 879-paper cache. Tightened the cleanliness gate against repository/portal stubs (Pure "Fingerprint", BibTeX/RIS export blocks, DigitalCommons/Dagstuhl/DSpace chrome, short no-body landings) + PDF text-quality heuristics. Vetted upgrade set: **73** (69 journal-clean HTML + 4 clean PDFs) in `chaos/genesis/coverage-upgrades.json` (12 HTML stubs demoted, 21 PDF extractions dropped, 9 borderline PDFs parked for a possible later micro-pass). **Conservative rejudge applied:** of the 73, the **4 in-graph** papers (W2088267428, W2114549073, W3092907164, W4396895867) were force-re-judged via multi-attach (cache bypassed for just those ids; no version bump) → **19 new grounding links across 15 concepts**, applied additively via `chaos/genesis/apply-links.js` (concept_links 598 → 617). The other 69 upgrades are out-of-graph and were deliberately skipped — recovering them (new grounding from already-vetted papers) is the leading edge of the scholarship-expansion phase.
...
5. **Multi-attach yield leak — FIXED (June 27, 2026).** `judgePaper` now salvages a malformed judgment envelope (strips fences/preamble/trailing garbage, repairs recoverable truncation) and, failing that, does one terse-reinforced retry before warn-and-drop; recovered results cache under the unchanged key. No verdict/rubric/version change. Proved out live on the rejudge run: 26 judgments → 1 salvaged, 0 still-failed. Counters (first-pass / salvaged / retried-ok / still-failed) print in both run summaries.
