...
- **Krius — the Scout** (`chaos/scout.js`). **NOT BUILT — next engine to build.** Bottom-up channel to propose new frontiers / adjectives / multi-parent placements within the existing spine. Will be the first engine built *into* an already-populated graph rather than from scratch.
...
**Next (Chaos):**
1. **Krius (Scout)** — build the third engine (the next major step). First engine built into an existing populated graph. Must uphold the promotion invariant (emit reviewable JSON; write name-based).
2. **Frontier object + frontier-driven run mode** in Ouranos (genesis is currently one-shot) — the interleaved categorize↔instantiate loop the steady-state design calls for.
...
4. **v0.x housekeeping / trim pass on `apply.js`** — drop the dormant payload (tunnels / papers-as-v0 / predictions / precision ledger) per the pure-hierarchy design, and fix the cosmetic stale print labels (the dry-run still prints "tunnel_links" and "value dispositions"; cosmetic only — behavior is correct). Retire `chaos/reason.js` + `chaos/source.js`.
