...
- **58d-1** — Concept page links experience. Rename `ConceptAnnotationPanel.jsx` → `ConceptLinksPanel.jsx` (with all callers updated). Add Top|New sort toggle to the Links tab (calling `GET /api/web-links/:edgeId?sort=top|new`). Add "Show other instances on this concept" expandable button on each link card (uses `GET /api/web-links/by-url` filtered to the current concept's edges, displays expandable list of comment snippets with click-to-jump). Add "Show instances across all concepts" expandable button on each link card (uses same endpoint, no concept filter, click-to-jump navigates to that concept page and auto-scrolls to the link). Wire the inline "+ Add Link" form to use the OG title auto-fetch — UX: when user submits with empty title, server fetches and returns the title; show "(auto-fetched, edit if you'd like)" hint and let user edit before final save. URL field uses minimal client validation (must start with http:// or https://, max 2048 chars). Defensive sweep at end of prompt to catch any orphan calls.
- **58d-2** — Superconcept aggregated view + Link Votes browser + sidebar polish. Replace `ComboTabContent.jsx` placeholder with aggregated links view (uses new `GET /api/combos/:id/links?sort=top|new`, same Top|New toggle, same link card style as Links panel — pull from a shared component if reasonable). Build new Link Votes browser component (replaces deleted `AnnotationVotesOverlay.jsx`) — list all `concept_links` the current user has upvoted, click-through navigates to the concept page and auto-scrolls to the specific link in the panel. New endpoint `GET /api/web-links/votes/me` may be needed if not already covered by 58b-2 (verify; add if absent). Sidebar polish — the 2x2 action-button grid that previously held "Graph Votes / Browse Corpuses / Browse Combos / Messages" now has only "Graph Votes / Browse Combos" left after 58c removed two; collapse to a sensible layout (1x2 horizontal or vertical stack). Rename "Graph Votes" sidebar label to "Link Votes" since that's the primary content now. Defensive runtime sweep at end of prompt.
