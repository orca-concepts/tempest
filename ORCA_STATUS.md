...
**Current Status:** Phase 65a complete (with two bug-fix follow-ups). Added share buttons to regular links and tunnel links (copying `/link/:id` and `/tunnel/:id` URLs to clipboard), client-side resolution of those URLs via new `GET /api/votes/web-links/:linkId/location` and `GET /api/tunnels/:tunnelLinkId/location` endpoints, and in-comment linkification of in-orca URLs via a new `LinkifiedText` component replacing `ClampedText` on all four user-authored comment surfaces (concept link comments, addenda, tunnel link comments, tunnel addenda). Two follow-up fixes shipped in the same arc: `65a-fix` (location endpoints returned path-to-concept-inclusive instead of path-to-parent, causing duplicate-concept-in-breadcrumb) and `65a-fix-2` (mount-time race between `loadAllTabs` default-activation and async deep-link resolution caused a brief flash of the wrong concept). Phase 64a (Google Safe Browsing URL check), 62c (zombie ORCID disconnect cleanup), and 63a (favicon + OG/Twitter Card preview tags) remain live.
...
Trailing punctuation in surrounding prose (commas, periods, parens) is NOT absorbed into the URL — match must end at a word boundary. Better to render a URL as plain text than to render a wrong URL as clickable. The grammar is intentionally narrow; URLs with garbage query params, non-numeric IDs, or any other variation do not linkify and do not generate mentions (Phase 65b will respect the same grammar).

**Future consumer (Phase 65b):** `parseInOrcaLinks(text)` is defined in `inOrcaLinks.js` and returns deduplicated `{ targetType, targetId, targetPath, fragment }` entries. It will be called at comment/addendum write time to populate the `comment_mentions` table.
...
- **In-orca URL regex duplicated (Phase 65a):** The grammar is defined in `backend/src/utils/inOrcaLinks.js` AND `frontend/src/components/LinkifiedText.jsx`. Both files carry comments noting that they must stay in sync character-for-character, but there is no enforcement. Two reasonable resolutions: (a) move the regex to a `shared/` directory imported by both backend (Node `require`) and frontend (Vite `import`); (b) add a build-time sync check (CI step or unit test that reads both files and compares regex strings). Low priority while the grammar is stable; revisit when 65b's mention parsing exposes the duplication to more change traffic.
...
- Phase 65b: Mentions panel — backreference index for in-orca URLs in comments/addenda. New `comment_mentions` join table, `parseInOrcaLinks` called at write time, Mentions panel UI on concepts, regular links, and tunnel links. Scaffolded by 65a (parser utility exists, grammar shared). **Next phase to ship.**
...
## Recent Commits (Phase 59 through 65a)
