...
- Seed account: `chaos-seed`, email `chaos-seed@orcaconcepts.org`, sentinel `orcid_id = 'CHAOS-SEED-ACCOUNT'` (non-digit, cannot collide with a real ORCID), login disabled via non-bcrypt password hash. **Resolve by `username='chaos-seed'`, never by hardcoded id** — the SERIAL id differs per environment (local dev: 25).
