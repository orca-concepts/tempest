...
// --rejudge-upgrades writes its OWN separate, reviewable proposal — it NEVER overwrites the two
// committed gaia proposals above. STEP 2: re-judge the in-graph full-text upgrades.
const OUT_REJUDGE_JSON = path.join(GENESIS_DIR, 'gaia-multiattach-rejudge-proposal.json');
const OUT_REJUDGE_MD = path.join(GENESIS_DIR, 'gaia-multiattach-rejudge-proposal.md');
...
// Targeted cache-bypass for the STEP-2 re-judge (--rejudge-upgrades). Paper ids in this set skip
// the propose/judgment cache READ (so their stale abstract-only entries are ignored and re-run
// against the new full text). It is empty for every other run, so normal --multiattach is
// unaffected, and only these ids' caches are refreshed — NO version bump, all other caches valid.
let FORCE_REJUDGE_IDS = new Set();
...
async function judgePaper(target, rec, systemText) {
  ...
  if (!FORCE_REJUDGE_IDS.has(rec.id) && fs.existsSync(cp)) {
    try {
...
// ----------------------------------------------------------------------------
// --multiattach --rejudge-upgrades (STEP 2, conservative): re-judge the IN-GRAPH full-text
// upgrades. Target set = ids in coverage-upgrades.json that ALREADY have ≥1 grounding link in the
// committed gaia proposals (grounded papers that multi-attach previously skipped for lack of full
// text). For each, FORCE Stage A propose + Stage B confirm against the new full text (skipping
// only that id's cache — no version bump), propose links ONLY to existing leaf concepts, dedup
// against the paper's existing committed links, and emit a SEPARATE reviewable proposal of the
// NEW links. --plan (default) prints the split + projected calls and spends nothing.
// Conservative scope: in-graph papers only, existing concepts only, additive links only.
// ----------------------------------------------------------------------------

function writeRejudgeJson(citedRecords, links) {
  const out = {
    generated_by: 'chaos/gaia.js --multiattach --rejudge-upgrades (STEP 2: in-graph full-text re-judge)',
    model: MODEL,
    effort: EFFORT,
    rubric_version: rubricVersion(),
    gaia_prompt_version: GAIA_PROMPT_VERSION,
    gaia_multiattach_version: GAIA_MULTIATTACH_VERSION, // unchanged — force is per-id, not a bump
    generated_at: new Date().toISOString(),
    note: 'Additive NEW links only, to EXISTING concepts, for in-graph upgraded papers. Separate '
      + 'from the committed gaia proposals (not overwritten). No DB writes (promotion invariant).',
    papers: citedRecords.map(trimPaperForProposal),
    links,
  };
  fs.writeFileSync(OUT_REJUDGE_JSON, JSON.stringify(out, null, 2), 'utf8');
}

function writeRejudgeMd({ perGroup, scanned, newLinkCount, targets }) {
  const L = [];
  L.push('# Chaos — Gaia multi-attach RE-JUDGE proposal (STEP 2, in-graph full-text upgrades)');
  L.push('');
  L.push(`**Generated by:** chaos/gaia.js --multiattach --rejudge-upgrades · model ${MODEL} · effort ${EFFORT} · rubric v${rubricVersion()}`);
  L.push('**Write target:** this file + chaos/genesis/gaia-multiattach-rejudge-proposal.json. The committed gaia proposals are NOT touched. No DB writes, no cutover.');
  L.push(`**Scope:** in-graph upgraded papers only (${scanned} re-judged of ${targets} targets) · existing concepts only · additive NEW links only.`);
  L.push(`**New links proposed:** ${newLinkCount} across ${perGroup.size} concept(s).`);
  L.push('');
  const groups = [...perGroup.values()].sort((a, b) => b.instances.length - a.instances.length);
  for (const g of groups) {
    L.push(`## ${g.name}  ${pathLabel(g.parentPath)}  (${g.instances.length})`);
    for (const inst of g.instances) {
      const url = inst.rec.best_oa_url || inst.rec.doi || inst.rec.openalex_id || '';
      L.push(`- [${inst.confidence}] ${inst.rec.id} — ${trunc(inst.claim, 200)}  (${url})`);
    }
    L.push('');
  }
  L.push('---');
  L.push('');
  L.push('**END — new candidate links only; review before folding into the cutover proposal.**');
  L.push('');
  fs.writeFileSync(OUT_REJUDGE_MD, L.join('\n'), 'utf8');
}

async function runRejudgeUpgrades() {
  const RUN = process.argv.includes('--run'); // --plan is the default
  if (!fs.existsSync(OUT_JSON) || !fs.existsSync(OUT_MA_JSON)) {
    throw new Error('committed gaia proposals not found (gaia-proposal.json / gaia-multiattach-proposal.json) — STEP 2 reads them as the grounded-paper source of truth.');
  }
  if (!fs.existsSync(COVERAGE_UPGRADES_PATH)) {
    throw new Error(`coverage-upgrades.json not found at ${path.relative(CHAOS_DIR, COVERAGE_UPGRADES_PATH)} — run --coverage-probe (STEP 1) first.`);
  }
  const primary = JSON.parse(fs.readFileSync(OUT_JSON, 'utf8'));
  const multi = JSON.parse(fs.readFileSync(OUT_MA_JSON, 'utf8'));
  const committedLinks = [...asArray(primary.links), ...asArray(multi.links)];

  // grounded paper ids + per-paper existing (concept|path) (for dedup) + already-linked targets.
  const grounded = new Set();
  const existingByPaper = new Map();
  const linkedByPaper = new Map();
  for (const l of committedLinks) {
    const pid = l.paper_id;
    if (!pid) continue;
    grounded.add(pid);
    const pairKey = `${normName(l.concept_name)}|${pathKey(asArray(l.parent_path))}`;
    if (!existingByPaper.has(pid)) existingByPaper.set(pid, new Set());
    existingByPaper.get(pid).add(pairKey);
    if (!linkedByPaper.has(pid)) linkedByPaper.set(pid, []);
    linkedByPaper.get(pid).push({ name: l.concept_name, parentPath: asArray(l.parent_path) });
  }

  const upgraded = asArray(JSON.parse(fs.readFileSync(COVERAGE_UPGRADES_PATH, 'utf8')).upgraded_ids);
  const inGraph = upgraded.filter((id) => grounded.has(id));
  const outGraph = upgraded.filter((id) => !grounded.has(id));

  const { targets: menuTargets } = loadTargets();
  const menuByKey = new Map();
  for (const t of menuTargets) menuByKey.set(`${normName(t.name)}|${pathKey(t.parentPath)}`, t);

  console.log('================ CHAOS GAIA — RE-JUDGE IN-GRAPH UPGRADES (STEP 2) ================');
  console.log(`mode: ${RUN ? 'RUN (forced re-judge of in-graph targets)' : 'PLAN (no model calls)'}  ·  model=${MODEL}  effort=${EFFORT}`);
  console.log(`total upgrades: ${upgraded.length}  ·  in-graph targets: ${inGraph.length}  ·  out-of-graph skipped: ${outGraph.length}`);
  console.log('\nin-graph target ids (existing links each):');
  for (const id of inGraph) console.log(`  ${id}  (${(existingByPaper.get(id) || new Set()).size} existing)`);
  console.log(`\nprojected calls: Stage A ${inGraph.length} propose (forced) · Stage B ≤ ${inGraph.length * 8} confirm (≤8/paper; abstain + dedup reduce).`);
  console.log('');

  if (!RUN) {
    console.log('PLAN COMPLETE — 0 model calls. Re-run with --run to execute the forced re-judge.');
    console.log('=================================================================================');
    return;
  }

  // Forced re-judge: bypass ONLY these ids' caches (no version bump). Reset recovery counters.
  FORCE_REJUDGE_IDS = new Set(inGraph);
  judgeRecovery.firstPass = 0; judgeRecovery.salvaged = 0; judgeRecovery.retriedOk = 0; judgeRecovery.stillFailed = 0;

  const rubric = loadRubric();
  const ctx = { systemJudge: judgeSystem(rubric), systemPropose: multiAttachProposeSystem(rubric) };
  ...
  for (const pid of inGraph) {
    const rec = loadPaperRecordById(pid);
    if (!rec) { console.warn(`  [skip] ${pid} — paper record missing on disk`); continue; }
    if (!(rec.full_text_available && rec.full_text)) { console.warn(`  [skip] ${pid} — no full_text`); skippedNoText++; continue; }
    scanned++;
    console.log(`[scan] ${rec.id}  ${trunc(rec.title, 60)}`);
    ...
    for (const cand of candidates) {
      ...
      seenPair.add(pairKey);
      const target = menuByKey.get(pairKey);
      if (!target) continue; // existing leaf concepts ONLY — never create a concept
      if ((existingByPaper.get(pid) || new Set()).has(pairKey)) continue; // dedup vs committed links

      const verdict = await judgePaper(target, rec, ctx.systemJudge);
      if (!verdict) continue;
      if (!(verdict.exhibits && verdict.comment && (verdict.confidence === 'high' || verdict.confidence === 'medium'))) continue;

      const url = rec.best_oa_url || rec.doi || rec.openalex_id || '';
      newLinks.push({ attribute: 'value', concept_name: target.name, parent_path: target.parentPath, url, claim: verdict.comment, paper_id: rec.id });
      citedRecords.set(rec.id, rec);
      const grp = perGroup.get(pairKey) || { name: target.name, parentPath: target.parentPath, instances: [] };
      grp.instances.push({ rec, claim: verdict.comment, confidence: verdict.confidence });
      perGroup.set(pairKey, grp);
      console.log(`      [+attach ${verdict.confidence}] ${target.name}  ${pathLabel(target.parentPath)}`);
    }
  }

  writeRejudgeJson([...citedRecords.values()], newLinks);
  writeRejudgeMd({ perGroup, scanned, newLinkCount: newLinks.length, targets: inGraph.length });

  console.log('\n================ RE-JUDGE SUMMARY ================');
  console.log(`in-graph targets re-judged: ${scanned}/${inGraph.length}` + (skippedNoText ? `  (skipped ${skippedNoText} with no full_text)` : ''));
  console.log(`new links proposed: ${newLinks.length} across ${perGroup.size} concept(s)`);
  for (const g of [...perGroup.values()].sort((a, b) => b.instances.length - a.instances.length)) {
    console.log(`  ${g.name}  ${pathLabel(g.parentPath)} — ${g.instances.length}`);
  }
  const rec = judgeRecoveryLine();
  if (rec) console.log(rec);
  console.log(`\nWrote ${path.relative(CHAOS_DIR, OUT_REJUDGE_JSON)} and ${path.relative(CHAOS_DIR, OUT_REJUDGE_MD)}`);
  console.log('Committed gaia proposals untouched. No DB writes. Review the new links next.');
  console.log('=================================================');
}
...
function printUsage(targets) {
  ...
  console.log('  node chaos/gaia.js --multiattach --rejudge-upgrades [--run]  # STEP 2: re-judge in-graph full-text upgrades');
...
async function main() {
  ...
  // The top-level offline plan. The STEP-2 re-judge has its OWN --plan (default) mode, so let that
  // combo fall through to the --multiattach dispatch below rather than printing the leaf plan.
  if (process.argv.includes('--plan')
    && !(process.argv.includes('--multiattach') && process.argv.includes('--rejudge-upgrades'))) {
    printPlan(targets);
    return;
  }
  ...
  if (process.argv.includes('--multiattach')) {
    if (process.argv.includes('--rejudge-upgrades')) {
      await runRejudgeUpgrades();
      return;
    }
